...
import { CheckCircle2, XCircle, Loader2, ArrowRight, Shield, Zap, BarChart3 } from "lucide-react";
...
export default function VerifyEmailPage() {
  ...
  const { verifyEmail, isAuthenticated } = useUserStore();
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} className="space-y-8">
              <div className="text-center space-y-5">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-2 leading-relaxed max-w-sm mx-auto">
                    Your account is fully activated. You now have access to all Codara features.
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-border bg-card/50 p-5 space-y-3">
                ...
                <div className="space-y-2.5">
                  {[
                    { step: "01", text: "Upload your SAS files to the workspace" },
                    { step: "02", text: "Configure target runtime and options" },
                    { step: "03", text: "Launch the AI conversion pipeline" },
                  ].map((item) => (
                    <div key={item.step} className="flex items-center gap-3">
                      <span className="text-[10px] font-mono font-bold text-accent/60 w-5">{item.step}</span>
              ...
              {!isAuthenticated && (
                <p className="text-center text-xs text-muted-foreground">
                  You can now <Link to="/login" className="text-accent font-medium hover:underline">sign in</Link> with your verified email
                </p>
              )}
            </motion.div>
          )}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-8">
              ...
              <div className="rounded-xl border border-border bg-card/50 p-5 space-y-3">
                ...
                <ul className="space-y-2 text-sm text-muted-foreground">
                  ...
                  <li className="flex items-start gap-2">
                    <span className="text-accent mt-0.5">&#8226;</span>
                    Links expire after 24 hours — request a new one from your dashboard
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-accent mt-0.5">&#8226;</span>
