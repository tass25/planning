...
async def run():
    ...
    for gf in sorted(GOLD_DIR.glob("gsh_*.gold.json")):
        ...
        gold_conds = [b for b in gold["blocks"] if b["partition_type"] == "CONDITIONAL_BLOCK"]
        if not gold_conds:
            continue
        ...
        det_conds = [e for e in events if e.partition_type.value == "CONDITIONAL_BLOCK"]
        
        for gc in gold_conds:
            gs = gc["line_start"]
            ge = gc["line_end"]
            matched = any(abs(d.line_start-gs)<=2 and abs(d.line_end-ge)<=2 for d in det_conds)
            if not matched:
                best = min(det_conds,
                    key=lambda d: abs(d.line_start-gs)+abs(d.line_end-ge),
                    default=None)
                if best:
    ...
    Path("debug_cond_out.txt").write_text("\n".join(out))
    print(f"Written {len(out)} lines to debug_cond_out.txt")
