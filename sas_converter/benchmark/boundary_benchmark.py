...
def evaluate_boundary_accuracy(results, gold_dir, line_tolerance=2):
    total_gold=0; correct=0; missed=[]; type_confusion={}; per_file={}
    for gf in sorted(Path(gold_dir).glob("*.gold.json")):
        with open(gf,encoding="utf-8") as f: gold=json.load(f)
        stem=Path(gold["file_path"]).stem; file_events=results.get(stem,[]); gold_blocks=gold.get("blocks",[]); fc=0
        for gb in gold_blocks:
            total_gold+=1; matched=False
            for ev in file_events:
                if ev.partition_type.value==gb["partition_type"] and abs(ev.line_start-gb["line_start"])<=line_tolerance and abs(ev.line_end-gb["line_end"])<=line_tolerance:
                    correct+=1; fc+=1; matched=True; break
            if not matched:
                missed.append({"file":stem,"gold_type":gb["partition_type"],"gold_start":gb["line_start"],"gold_end":gb["line_end"]})
                cl=min(file_events,key=lambda e:abs(e.line_start-gb["line_start"]),default=None)
                if cl:
...
async def _run_corpus(gold_dir, enable_llm):
    ...
    for sf in sas_files:
        try:
            fm=_make_file_meta(sf); pairs=await run_streaming_pipeline(fm)
            if enable_llm:
                agent=BoundaryDetectorAgent(trace_id=uuid4()); events=await agent.process(pairs,fm.file_id)
            else:
                events=BoundaryDetector().detect(pairs,fm.file_id)
            results[sf.stem]=events; print(f"  {sf.name:<50} {len(events):>3} block(s)")
        except Exception as exc:
            print(f"  [WARN] {sf.name}: {exc}"); results[sf.stem]=[]
    return results
...
async def main():
    ...
    parser.add_argument("--llm",action="store_true",help="Enable Groq LLM for ambiguous blocks (needs GROQ_API_KEY)")
    parser.add_argument("--verbose",action="store_true")
    args=parser.parse_args()
    gold_dir=Path(args.gold)
    if not gold_dir.exists(): print(f"[ERROR] Not found: {gold_dir}"); raise SystemExit(1)
    if args.llm:
        import os
        if not os.environ.get("GROQ_API_KEY"): print("[ERROR] --llm needs GROQ_API_KEY"); raise SystemExit(1)
    ...
    if args.verbose:
        ...
        if report["confusion"]:
            ...
            for ps,cnt in sorted(report["confusion"].items(),key=lambda x:-x[1]): print(f"  {ps}  ({cnt}x)")
        if report["missed"]:
            print("\n-- Missed blocks --")
            for m in report["missed"][:20]: print(f"  {m['file']}: {m['gold_type']} L{m['gold_start']}-{m['gold_end']}")
            if len(report["missed"])>20: print(f"  ... and {len(report['missed'])-20} more")
