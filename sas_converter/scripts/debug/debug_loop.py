...
async def run():
    for gf in sorted(GOLD_DIR.glob("gsh_*.gold.json")):
        ...
        gold_blocks = [b for b in gold["blocks"] if b["partition_type"] == "LOOP_BLOCK"]
        if not gold_blocks:
            continue
        ...
        det = [e for e in events if e.partition_type.value == "LOOP_BLOCK"]
        ...
        for gb in gold_blocks:
            ...
            if not matched:
                ...
                if best:
                    print(f"{stem}: gold L{gs}-{ge} -> det L{best.line_start}-{best.line_end} ed={best.line_end-ge:+d}")
                else:
