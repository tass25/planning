...
async def run():
    ...
    gold = json.load(open(f"knowledge_base/gold_standard/{stem}.gold.json"))
    ...
    print("=== GOLD PROC_BLOCKs ===")
    for p in gold_procs:
        print(f"  gold L{p['line_start']}-{p['line_end']}")
    
    print("\n=== DETECTED PROC_BLOCKs ===")
    for d in det_procs:
        print(f"  det  L{d.line_start}-{d.line_end}")
    ...
    for gp in gold_procs:
        ...
        if not matched:
            ...
            if best:
                print(f"  gold L{gs}-{ge} -> best_det L{best.line_start}-{best.line_end} sd={best.line_start-gs:+d} ed={best.line_end-ge:+d}")
            else:
                print(f"  gold L{gs}-{ge} -> NONE")
