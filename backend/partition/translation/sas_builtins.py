...
_BUILTINS: dict[str, tuple[str, str]] = {
    # ── String functions ──────────────────────────────────────────────────────
    "SUBSTR": ("str[start:end]", "SAS is 1-indexed; Python is 0-indexed. SUBSTR(s,2,3) → s[1:4]"),
    "SUBSTRING": ("str[start:end]", "Alias for SUBSTR"),
    "LENGTH": ("len(str) or df['col'].str.len()", "SAS returns the length WITHOUT trailing spaces"),
    "TRIM": (".str.rstrip()", "SAS TRIM removes only TRAILING spaces"),
    "STRIP": (".str.strip()", "SAS STRIP removes both leading and trailing spaces"),
    "LEFT": (".str.lstrip()", "SAS LEFT removes leading spaces"),
    "COMPRESS": (
        ".str.replace(r'[chars]', '', regex=True)",
    ...
    "UPCASE": (".str.upper()", "Direct equivalent"),
    "LOWCASE": (".str.lower()", "Direct equivalent"),
    "PROPCASE": (".str.title()", "SAS PROPCASE capitalises first letter of each word"),
    "CATS": (
        "''.join([str(x) for x in [...] if pd.notna(x)])",
        "Concatenate stripping trailing spaces; missing values excluded",
    ),
    "CATX": (
        "sep.join([str(x) for x in [...] if pd.notna(x)])",
    ...
    "CHAR": ("chr(n)", "Returns character for ASCII code"),
    "RANK": ("ord(char)", "Returns ASCII rank/code of a character"),
    "VERIFY": (
        "any(c not in allowed for c in s)",
    ...
    "ROUND": (
        "round(x, d)  /  df['col'].round(d)",
        "SAS ROUND(x,0.01) rounds to nearest 0.01; Python round() uses banker's rounding",
    ),
    "INT": ("int(x)  /  np.trunc(x)", "SAS INT truncates toward zero; use int() or np.trunc()"),
    ...
    "MAX": (
        "max(x, y)  /  df[['a','b']].max(axis=1)",
        "SAS MAX ignores missing; pandas max() also ignores NaN by default",
    ),
    "MIN": (
        "min(x, y)  /  df[['a','b']].min(axis=1)",
        "SAS MIN ignores missing; pandas min() also ignores NaN by default",
    ),
    "SUM": (
        "sum([x, y])  /  df[['a','b']].sum(axis=1)",
        "SAS SUM ignores missing values; pandas sum(axis=1, skipna=True) too",
    ),
    "MEAN": ("np.mean([x, y])  /  df[['a','b']].mean(axis=1)", "Row-wise mean ignoring missing"),
    "N": ("df[['a','b']].count(axis=1)", "Count of non-missing values across columns"),
    "NMISS": ("df[['a','b']].isnull().sum(axis=1)", "Count of missing values across columns"),
    "CMISS": (
        "df[['a','b']].isnull().sum(axis=1)",
        "Count of missing (char or numeric) — same as NMISS in pandas context",
    ),
    "MISSING": ("pd.isna(x)", "Returns True if value is missing (NaN or None)"),
    "COALESCE": (
        "df[['a','b','c']].bfill(axis=1).iloc[:,0]",
        "Returns first non-missing value; pandas bfill across columns then take first",
    ),
    "COALESEC": ("df[['a','b','c']].bfill(axis=1).iloc[:,0]", "Character version of COALESCE"),
    "RAND": (
        "np.random.rand()  /  rng.uniform(0,1)",
    ...
    "DATE": ("pd.Timestamp.today().normalize()", "Alias for TODAY()"),
    "DATETIME": ("pd.Timestamp.now()", "Returns current datetime"),
    "TIME": (
        "pd.Timestamp.now().time()",
        "Returns current time as seconds since midnight in SAS; use datetime.now().time()",
    ),
    "YEAR": (".dt.year", "Extracts year from a SAS date variable — use pandas .dt.year"),
    "MONTH": (".dt.month", "Extracts month (1-12)"),
    "DAY": (".dt.day", "Extracts day of month"),
    "HOUR": (".dt.hour", "Extracts hour"),
    "MINUTE": (".dt.minute", "Extracts minute"),
    "SECOND": (".dt.second", "Extracts second"),
    "WEEKDAY": (
        ".dt.dayofweek + 1",
        "SAS: 1=Sunday..7=Saturday. pandas: 0=Monday..6=Sunday. Add 1 and adjust.",
    ),
    "QTR": ("((df['date'].dt.month - 1) // 3) + 1", "Quarter number 1-4"),
    "MDY": (
        "pd.Timestamp(year=y, month=m, day=d)",
        "MDY(month, day, year) → pd.Timestamp(year=y, month=m, day=d)",
    ),
    "YMD": ("pd.Timestamp(year=y, month=m, day=d)", "Alias MDY with different arg order"),
    "HMS": (
        "pd.Timedelta(hours=h, minutes=m, seconds=s)",
        "Creates a time value from hours, minutes, seconds",
    ),
    "DHMS": (
        "pd.Timestamp + pd.Timedelta(hours=h, minutes=m, seconds=s)",
        "Creates datetime from date + time components",
    ),
    "DATEPART": (".dt.normalize()  /  .dt.date", "Extracts date part from SAS datetime"),
    "TIMEPART": (".dt.time", "Extracts time part from SAS datetime"),
    "INTCK": (
        "(end_date - start_date) / pd.Timedelta('1D') for 'DAY'; use relativedelta for months/years",
        "INTCK('MONTH',start,end) counts interval boundaries crossed — use relativedelta",
    ),
    "INTNX": (
        "start_date + pd.DateOffset(months=n) for 'MONTH'; adjust for alignment",
        "INTNX('MONTH',date,n,'B') advances n months to beginning; use pd.DateOffset",
    ),
    "DATDIF": ("(date2 - date1).days", "Difference in days between two SAS dates"),
    "YRDIF": (
        "(date2 - date1).days / 365.25",
        "Approximate years between dates; for AGE use relativedelta",
    ),
    # ── Statistical functions ─────────────────────────────────────────────────
    "PROBIT": ("scipy.stats.norm.ppf(p)", "Inverse normal CDF (probit)"),
    "PROBNORM": ("scipy.stats.norm.cdf(x)", "Normal CDF P(Z <= x)"),
    "PROBCHI": ("scipy.stats.chi2.cdf(x, df)", "Chi-squared CDF"),
    "PROBF": ("scipy.stats.f.cdf(x, df1, df2)", "F distribution CDF"),
    "PROBT": ("scipy.stats.t.cdf(x, df)", "Student-t CDF"),
    "TINV": ("scipy.stats.t.ppf(p, df)", "Inverse t-distribution"),
    "FINV": ("scipy.stats.f.ppf(p, df1, df2)", "Inverse F-distribution"),
    "CINV": ("scipy.stats.chi2.ppf(p, df)", "Inverse chi-squared distribution"),
    # ── Utility ───────────────────────────────────────────────────────────────
    "PUT": (
        "format() or strftime()",
    ...
    "LAG2": (".shift(2)", "Two-row lag"),
    "LAG3": (".shift(3)", "Three-row lag"),
    "DIF": (".diff(1)", "DIF(var) = var - LAG(var); use .diff(1)"),
    "FIRST": (
        "df.groupby(grp).cumcount() == 0",
        "FIRST.var equivalent: first row in each BY group",
    ),
    "LAST": (
        "df.groupby(grp).cumcount(ascending=False) == 0",
