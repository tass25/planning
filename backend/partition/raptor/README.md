...
| `summarizer.py` | `ClusterSummarizer` — 3-tier LLM fallback: Nemotron (Ollama) → Azure GPT-5.4-mini → Groq Llama-3.3-70B → heuristic |
...
list[PartitionIR] (from Chunking Layer)
  ...
  ClusterSummarizer
    -> Nemotron (Ollama) → Azure GPT-5.4-mini → Groq 70B → heuristic fallback
