    ...
    BugCase(
        ...
        sas=textwrap.dedent("""\
            proc sort data=customer_addresses nodupkey;
              by customer_id;
            run;"""),
        buggy_py=("customer_addresses = customer_addresses.sort_values('customer_id')"),
        correct_py=(
...
def _run_case(case: BugCase, agent: Z3VerificationAgent) -> None:
    ...
    case.buggy_z3_hint = (
        (br.counterexample.get("hint", "") or br.counterexample.get("fix", ""))
        if br.counterexample
        else ""
    )
...
def _print_case(case: BugCase, idx: int, total: int) -> None:
    ...
    print("\n  +-- WITHOUT Z3 (syntax + exec validation only) ---------------+")
    print("  |  Result : PASS  ->  translation ships as SUCCESS            |")
    print("  |  Bug    : UNDETECTED  (semantic error enters production)    |")
    print("  +-------------------------------------------------------------+")
...
def _print_summary(cases: list[BugCase]) -> None:
    ...
    W = 72
    print(f"\n{'=' * W}")
    print("  Z3 EFFECT SUMMARY  --  Before vs After")
    print(f"{'=' * W}\n")
