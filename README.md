...
7. **L2-E (Persistence & Index)** — `PersistenceAgent` writes to SQLite/Postgres/Parquet. `RAPTORWriter` pushes tree nodes to LanceDB. `IndexAgent` builds a NetworkX dependency graph, performs **SCC condensation** (handling circular `%INCLUDE` chains), and writes to a Kuzu embedded graph database with `DEPENDS_ON` and `MACRO_CALLS` relationships. Redis checkpoints every 50 blocks (TTL 24h) for fault tolerance.
8. **Layer 3 (Translation + Validation)** — `TranslationAgent` detects failure modes, queries the KB for similar examples (k=5), and routes to Llama 8B (low risk) or Llama 70B via Groq (moderate/high risk) to produce Python or PySpark output based on `target_runtime`. Cross-verification ensures correctness. `ValidationAgent` then executes the generated code via `exec()` on a synthetic 100-row DataFrame in a sandboxed namespace (restricted builtins, 5 s timeout, no I/O). It checks: no exceptions, expected columns present, date-range plausibility, merge row-count bounds. On pass → L4; on fail with retries < 2 → re-translate; on fail with retries ≥ 2 → PARTIAL; structural_only blocks auto-pass.
...
Enterprise SAS projects often have **circular `%INCLUDE` chains** (A includes B includes C includes A). Standard DAG algorithms fail on cycles. The `IndexAgent` collapses them into **super-nodes** via Strongly Connected Component (SCC) condensation before writing to Kuzu.
...
flowchart TB
  ...
  subgraph HOP["Dynamic Hop Cap"]
    ...
    CYPHER["all Kuzu queries use\nMATCH (a)-[*1..{cap}]->(b)"]
    LONGEST --> CAP --> CYPHER
  end

  subgraph KUZU_W["Kuzu Write"]
    NODE_W["CREATE Partition nodes\npartition_id, type, risk_level, scc_id"]
    REL_DEP["CREATE DEPENDS_ON rels\ndep_type: dataset | libref | join_input"]
    REL_MAC["CREATE MACRO_CALLS rels\nmacro_name property"]
...
2. **SCC Condensation** — `nx.strongly_connected_components()` identifies all dependency cycles.
   ...
   - `nx.condensation()` replaces each SCC with a single super-node, **guaranteeing an acyclic graph** for downstream Kuzu queries.

3. **Dynamic Hop Cap** — `nx.dag_longest_path_length()` computes the longest dependency chain in the condensed graph. The hop cap is set to `min(longest, 10)` and stored in `config/project_config.yaml`. All Kuzu Cypher queries use `MATCH (a)-[*1..{cap}]->(b)` to bound traversal depth.

4. **Kuzu Write** — Partition nodes are created with `partition_id`, `type`, `risk_level`, and `scc_id`. Two relationship types are written: `DEPENDS_ON` (with `dep_type` property) and `MACRO_CALLS` (with `macro_name` property).
...
> **Why this matters for translation:** Partitions with the **same `scc_id`** form a circular dependency and **must be translated together**, not independently. Layer 3's `TranslationAgent` groups same-SCC partitions into a single translation batch, providing all members as context to the LLM simultaneously.
...
flowchart TB
  ...
  subgraph L3["Layer 3 · TranslationAgent + ValidationAgent"]
    ...
    KB_Q["KBQueryClient.query_similar()\nk=5 + failure_mode filter"]
    ROUTE{"risk_level\nrouting"}
    LLM_8B["Ollama Llama 8B\nLOW risk blocks"]
    LLM_70B["Groq Llama 70B\nMODERATE / HIGH blocks\ntemp=0.1 for HIGH"]
    HR["HUMAN_REVIEW\nskip LLM → stub"]
    VERIFY["Prompt C cross-verify\nMODERATE + HIGH only"]
    RETRY_T["retry × 2 with issues"]
    VA_D["ValidationAgent\nexec() synthetic 100-row DataFrame\nsandboxed namespace · 5s timeout\ncheck: columns, dates, merge bounds"]
    VA_ROUTE{"validation\nresult"}
    CR_OUT[("conversion_results\nstatus · llm_confidence\nfailure_mode_flagged · model_used\ntarget_runtime · validation_result")]
    SCC_CHK --> FM_DET --> KB_Q --> ROUTE
    ROUTE -->|LOW| LLM_8B
    ROUTE -->|MOD/HIGH| LLM_70B
    ROUTE -->|UNCERTAIN| HR
    LLM_8B & LLM_70B --> VERIFY
    VERIFY -->|fail| RETRY_T --> VERIFY
    VERIFY --> VA_D
    HR --> CR_OUT
    VA_D --> VA_ROUTE
