...
flowchart TB
  ...
  subgraph ORCH["⚙️ Orchestration & DevOps"]
    PO["PartitionOrchestrator\nLangGraph StateGraph\n+ Azure Container App Deployment\n+ GitHub Actions CI/CD"]
    AUDIT[("DuckDB + Azure Monitor\nllm_audit + App Insights tracing\n+ feedback_log + conversion_reports\n(Azure Static Web Apps)")]
  end
...
4. **L2-C (RAPTOR Chunking)** — The core innovation. `BoundaryDetectorAgent` uses a Lark LALR grammar for **80% of cases** (rule-based). The remaining **20% ambiguous** boundaries are resolved by **Azure OpenAI GPT-4o**, with a tiktoken token guard. `PartitionBuilderAgent` creates partition objects of 9 canonical types. Partitions are then embedded (Nomic Embed v1.5, 768-dim), clustered (GMM with BIC), summarized (**Azure OpenAI GPT-4o-mini**, with hash caching), and assembled into a **RAPTOR tree** (leaf → L1 → L2 → root). Recursion stops when BIC delta < 0.01.
...
8. **Layer 3 (Translation + Validation)** — `TranslationAgent` selects one of **three RAG paradigms** per partition: **Static RAG** (LOW risk, no deps → fixed k=3, leaf level, **Azure GPT-4o-mini**, no cross-verify), **GraphRAG** (cross-file deps or SCC → `nx.ancestors()` for upstream siblings, k=5, cluster level, **Azure GPT-4o-mini**, cross-verify, generation retry), or **Agentic RAG** (MOD/HIGH risk → adaptive k 5→8→11, **Azure GPT-4o**, query reformulation on failure, RAPTOR level escalation leaf→cluster→root, retrieval retry, final escalation combines root + graph context). All paradigms feed into `ValidationAgent`, which executes the generated code via `exec()` on a synthetic 100-row DataFrame in a sandboxed namespace (restricted builtins, 5 s timeout, no I/O). It checks: no exceptions, expected columns present, date-range plausibility, merge row-count bounds. On pass → L4; on fail with retries < 2 → re-translate; on fail with retries ≥ 2 → PARTIAL; structural_only blocks auto-pass.
9. **Layer 4 (Merge + Report)** — `ImportConsolidator` deduplicates imports. `DependencyInjector` maps SAS datasets to Python variables. `ScriptMerger` assembles final `.py` files with `ast.parse()` syntax validation. Untranslated blocks get `TODO` stubs. `ReportAgent` then generates a Markdown + HTML conversion report containing CodeBLEU scores, validation results, and dependency graphs. Outputs `output/{name}_converted.py` + `{name}_report.md/html`. These HTML reports are deployed securely to **Azure Static Web Apps** for the final presentation.
10. **Continuous Learning & Telemetry** — `FeedbackIngestionAgent` accepts corrections. `ConversionQualityMonitor` tracks success rates and failure-mode distribution, emitting metrics directly to **Azure Monitor (Log Analytics)** for live dashboarding. `RetrainingTrigger` recalibrates the complexity model when ECE drifts.
11. **Orchestration, CI/CD, & Cloud Deployment** — `PartitionOrchestrator` runs via LangGraph. The entire orchestration logic pushes states to **Azure App Insights** for tracing. The project uses **GitHub Actions** for CI/CD testing, and the API itself is dockerized and deployed to a serverless **Azure Container App** (scaling to zero to save credits).
...
| **11** | Merge Layer (L4) + ReportAgent + Continuous Learning + Azure Monitor integration + KB to 330 pairs | P2 | ImportConsolidator · ScriptMerger · Log Analytics telemetry push |
| **12** | Ablation study: RAPTOR vs flat · CI/CD setup via GitHub Actions | P2 | Automated testing pipeline on push · Extracted hit-rate benchmarks |
| **13** | Defense prep: Azure Container App deployment + Azure Static Web Apps | P3 | Hosted API and web reports for interactive Live Demo validation |
| **14** | Buffer: polish ablation plots, add 50 more KB pairs, README | P3 | Dockerfile & docker-compose.yml finalized |
