...
\begin{enumerate}
  \item \textbf{NaN injection (8\% of numeric cells)}: exposes SAS
    missing-value (\texttt{.}) vs pandas \texttt{NaN} arithmetic divergence.
    In SAS, \texttt{. + 5 = .}; in pandas, \texttt{NaN + 5 = NaN}. These
    agree for addition but may diverge for comparison operators
    (\texttt{. < 0} is false in SAS; \texttt{NaN < 0} is false in pandas ---
    but the reason differs and edge cases can diverge).
