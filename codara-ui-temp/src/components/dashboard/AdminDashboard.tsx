...
import { mockAnalytics, mockAuditLogs, mockUsers, mockSystemServices } from "@/lib/mock/data";
...
export default function AdminDashboard() {
  ...
  const totalCost = mockAuditLogs.reduce((a, l) => a + l.cost, 0);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-panel p-5 lg:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={mockAnalytics.slice(-14)}>
              <defs>
        ...
        <div className="glass-panel p-5">
          ...
          <div className="mt-5 pt-4 border-t border-border">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-semibold text-foreground">{mockUsers.filter((u) => u.status === "active").length}</span>
            </div>
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-semibold text-foreground">{mockAuditLogs.length}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="glass-panel p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={mockAnalytics.slice(-10)}>
              <XAxis dataKey="date" tick={{ fontSize: 10, fill: "hsl(var(--chart-muted))" }} tickFormatter={(v) => v.slice(5)} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 10, fill: "hsl(var(--chart-muted))" }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12, color: "hsl(var(--foreground))" }} />
