  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <StatCard title="Success Rate" value={`${((completed / total) * 100).toFixed(0)}%`} icon={CheckCircle} variant="success" />
        <StatCard title="Partial Rate" value={`${((partial / total) * 100).toFixed(0)}%`} icon={AlertTriangle} />
        <StatCard title="Failure Rate" value={`${((failed / total) * 100).toFixed(0)}%`} icon={XCircle} variant="destructive" />
